...
Bridge:
    - Typically 4 tasks, one (or multiple) logs per task.
        - Single-run: `eval_env_<TaskName>_gpuX.log`
        - Repeats: `eval_env_<TaskName>_repN_gpuX.log` (N=1..K)
        - Repeats will be aggregated automatically by `TaskName`.
Fractal:
...
def _bridge_test_id_from_filename(filename: str) -> Optional[str]:
    # Support both single-run and repeated runs:
    # - eval_env_<TaskName>_gpuX.log
    # - eval_env_<TaskName>_repN_gpuX.log
    m = re.match(r"^eval_env_(.+?)(?:_rep\d+)?_gpu\d+\.log$", filename)
    if not m:
        return None
    return m.group(1)
...
def main() -> None:
    ...
    parser.add_argument(
        "--save",
        action="store_true",
        help="Save the summary to eval_dir/summary.json.",
    )
