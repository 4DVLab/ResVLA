...
def _ensure_dict_attr(args, key):
    if args.additional_env_build_kwargs is None:
        args.additional_env_build_kwargs = {}
    value = args.additional_env_build_kwargs.setdefault(key, {})
    if not isinstance(value, dict):
        value = {}
        args.additional_env_build_kwargs[key] = value
    return value


def _apply_runtime_env_overrides(args):
    # Keep start_simpler_env.py close to upstream; machine-specific renderer
    # settings are interpreted here as eval runtime configuration.
    render_device = os.getenv("SIMPLER_RENDER_DEVICE")
    if render_device:
        renderer_kwargs = _ensure_dict_attr(args, "renderer_kwargs")
        renderer_kwargs["offscreen_only"] = True
        if render_device != "auto":
            renderer_kwargs["device"] = render_device

    if os.getenv("SIMPLER_DISABLE_RT_DENOISER", "0") == "1":
        render_config = _ensure_dict_attr(args, "render_config")
        render_config["rt_use_denoiser"] = False
