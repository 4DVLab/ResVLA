...
def print_freeze_status(self):
    ...
    status_dict = defaultdict(
        lambda: {
            "Frozen": {"tensors": 0, "numel": 0},
            "Trainable": {"tensors": 0, "numel": 0},
            "params": [],
        }
    )
    for full_name, param in self.named_parameters():
        ...
        status_dict[top_module][state]["tensors"] += 1
        status_dict[top_module][state]["numel"] += param.numel()

    print("=== module parameter freezing status ===")
    for top_module, info in status_dict.items():
        frozen_tensors = info["Frozen"]["tensors"]
        trainable_tensors = info["Trainable"]["tensors"]
        frozen_numel = info["Frozen"]["numel"]
        trainable_numel = info["Trainable"]["numel"]

        if frozen_tensors > 0 and trainable_tensors == 0:
            # all frozen
            print(f"{top_module:40s}  |  all Frozen ({frozen_numel:,} params, {frozen_tensors} tensors)")
        elif trainable_tensors > 0 and frozen_tensors == 0:
            # all trainable
            print(f"{top_module:40s}  |  all Trainable ({trainable_numel:,} params, {trainable_tensors} tensors)")
        else:
            # mixed state, first print the module name summary, then list the state of each parameter
            print(
                f"{top_module:40s}  |  mixed state → "
                f"Frozen: {frozen_numel:,} params/{frozen_tensors} tensors, "
                f"Trainable: {trainable_numel:,} params/{trainable_tensors} tensors"
            )
