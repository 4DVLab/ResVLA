...
_ACTION_TOKEN_MAX = 153716 # here only for fast_tokenizer, see resVLA/model/modules/vlm/tools/add_qwen_special_tokens/README.md
...
class _QWen3_VL_Interface(nn.Module):
    ...
    def build_qwenvl_inputs(self, images, instructions, solutions=None, **kwargs):
        ...
        if solutions is not None: #  here only for fast_tokenizer now. 
            ...
            action_token_max = _ACTION_TOKEN_MAX # here only for fast_tokenizer, see resVLA/model/modules/vlm/tools/add_qwen_special_tokens/README.md
            ...
            for i in range(labels.size(0)):
                ...
                if nonzero_indices.numel() > 0:
                    ...
                    seq[:] = IGNORE_INDEX
                    RuntimeWarning (f"action token are on in yout tokenizer, plz see resVLA/model/modules/vlm/tools/add_qwen_special_tokens/README.md.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--debug", action="store_true", help="Wait for a debugpy client before running.")
    args, clipargs = parser.parse_known_args()

    cfg = OmegaConf.load(args.config_yaml)
    if args.debug or cfg.get("is_debug", False) or os.getenv("DEBUG", "").lower() in {"1", "true", "yes"}:
        import debugpy
