...
def _iter_unique_named_parameters(model):
    seen = set()
...
def count_parameters(model):
    ...
    for name, param in _iter_unique_named_parameters(model):
        ...
        module_stats = by_module.setdefault(module_name, {"total": 0, "trainable": 0})
        module_stats["total"] += numel
        if param.requires_grad:
            module_stats["trainable"] += numel
...
def format_parameter_count(num_params):
    if num_params >= 10**9:
        return f"{num_params / 10**9:.3f}B"
    return f"{num_params / 10**6:.3f}M"
...
def build_param_lr_groups(model, cfg):
    ...
    for module_name, lr in lr_cfg.items():
        ...
        try:
            ...
            logger.warning(f"⚠️ module path `{module_name}` not found in vla")
...
class TrainerUtils:
    ...
    def print_trainable_parameters(model):
        ...
        num_params = param_stats["total"]
        num_trainable_params = param_stats["trainable"]
        num_frozen_params = param_stats["frozen"]
        trainable_ratio = 100 * num_trainable_params / num_params if num_params else 0.0
        print(
            "# Parameters: "
            f"{format_parameter_count(num_params)} Total, "
            f"{format_parameter_count(num_trainable_params)} Trainable, "
            f"{format_parameter_count(num_frozen_params)} Frozen "
            f"({trainable_ratio:.2f}% trainable)"
        )
        for module_name, module_stats in sorted(param_stats["by_module"].items()):
            module_total = module_stats["total"]
            module_trainable = module_stats["trainable"]
            module_ratio = 100 * module_trainable / module_total if module_total else 0.0
            print(
                f"  - {module_name}: "
                f"{format_parameter_count(module_total)} total, "
                f"{format_parameter_count(module_trainable)} trainable "
                f"({module_ratio:.2f}%)"
            )
    ...
    def _get_latest_checkpoint(self, checkpoint_dir):
        ...
        # Collect checkpoint files that match the naming rule and end with .pt
        checkpoints = [
            f for f in os.listdir(checkpoint_dir) 
            if re.match(r"steps_(\d+)_pytorch_model\.pt$", f)  # Keep the $ anchor so the name ends with .pt
            and os.path.isfile(os.path.join(checkpoint_dir, f))  # Ensure the path is a file
        ]
        ...
        # Sort by step number and select the latest checkpoint
